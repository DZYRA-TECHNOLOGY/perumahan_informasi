...
export default function DataTable({
  ...
  // Cek apakah tabel bisa digeser ke kanan (untuk indikator fade)
  const checkScroll = () => {
  ...
  return (
    <div>
      ...
      <div className="relative card print:hidden">
        <div
          ref={scrollRef}
          className="overflow-x-auto touch-pan-x overscroll-x-contain [-webkit-overflow-scrolling:touch]"
        >
          <table className="w-full min-w-max text-sm">
            <thead className="bg-white/5 text-left muted">
              <tr>
                {columns.map((c) => (
                  <th
                    key={c.key}
                    className={`whitespace-nowrap px-4 py-3 font-medium ${c.align === "right" ? "text-right" : c.align === "center" ? "text-center" : ""}`}
                  >
                    {c.label}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-white/5">
              {slice.length === 0 && (
                <tr>
                  <td className="px-4 py-6 muted" colSpan={columns.length}>
                    Tidak ada data.
                  </td>
                </tr>
              )}
      ...
      {/* Petunjuk kecil khusus mobile */}
      {canScrollRight && (
        <p className="mt-1.5 flex items-center gap-1 text-[11px] muted sm:hidden">
          <span>← Geser tabel untuk lihat kolom lainnya →</span>
        </p>
      )}

      <div className="mt-3 flex items-center justify-between text-sm no-print">
        <span className="muted">
          {filtered.length} data · hal {cur}/{pages}
        </span>
