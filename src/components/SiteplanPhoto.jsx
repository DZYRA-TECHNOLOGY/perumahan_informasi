...
import { petaBlokMeta, kavlingList, STATUS, rekapStatus } from '../data/siteplan.js'
...
export default function SiteplanPhoto() {
  ...
  const blokInfo = (kode) => {
    ...
    const byStatus = cells.reduce((a, c) => ((a[c.status] = (a[c.status] || 0) + 1), a), {})
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <button
          ...
          className={`chip ring-1 transition ${
            kalibrasi ? 'bg-orange-500 text-white ring-orange-500' : 'text-zinc-200 ring-white/15 hover:bg-white/5'
          }`}
        >
          {kalibrasi ? '✓ Mode Kalibrasi (klik gambar)' : 'Kalibrasi posisi hotspot'}
        </button>
        {coord && (
      ...
      <div className="relative overflow-hidden rounded-2xl ring-1 ring-white/10">
          ...
          <div className="grid aspect-[16/10] place-items-center bg-white/[0.03] p-8 text-center">
            <div>
              ...
              <p className="mt-1 text-sm muted">
                Simpan file siteplan sebagai <code className="rounded bg-white/10 px-1.5 py-0.5 text-orange-400">public/siteplan.png</code> lalu muat ulang halaman.
              </p>
            </div>
          </div>
        )}
        ...
        {imgOk && Object.entries(HOTSPOTS).map(([kode, pos]) => (
          <button
            key={kode}
        ...
        return (
          <div className="mt-4 flex flex-col gap-4 card p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm muted">{total} kavling · DP {rp(meta.dp)}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(STATUS).filter(([k]) => byStatus[k]).map(([k, s]) => (
                <span key={k} className="inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1 text-xs ring-1 ring-white/10">
                  <span className="h-2.5 w-2.5 rounded-full" style={{ background: s.color }} />
                  {s.label}: {byStatus[k]}
                </span>
              ))}
            </div>
            <button onClick={() => setActive(null)} className="text-xs muted hover:text-zinc-100">Tutup ✕</button>
          </div>
        )
      })()}

      <p className="mt-3 text-xs muted">
        Klik titik blok (A–F) di atas gambar untuk melihat rekap status. Total terpetakan: {kavlingList.length} kavling.
        {' '}Distribusi: {Object.entries(rekapStatus).map(([k, v]) => `${k} ${v}`).join(' · ')}.
      </p>
    </div>
  )
}
